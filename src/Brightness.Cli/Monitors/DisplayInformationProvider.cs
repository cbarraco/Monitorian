...
internal static class DisplayInformationProvider
{
	...
	private static T GetActivationFactory<T>(string activatableClassId) where T : class
	{
		...
		try
		{
			...
			if (RoGetActivationFactory(hstring, ref iid, out factoryPtr) != 0)
				return null;
		...
		{
			_isActivationFactoryAvailable = false;
			return null;
		}
		finally
		{
	...
	{
		if (!_isActivationFactoryAvailable || (_dispatcherQueueController is null))
			return;
