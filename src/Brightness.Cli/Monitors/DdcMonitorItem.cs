...
internal class DdcMonitorItem(
	...
	MonitorCapability capability) : MonitorItem(
		...
		isReachable: true)
{
	private readonly SafePhysicalMonitorHandle _handle = handle ?? throw new ArgumentNullException(nameof(handle));
	private readonly MonitorCapability _capability = capability ?? throw new ArgumentNullException(nameof(capability));
