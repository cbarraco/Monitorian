...
internal class DisplayConfig
{
		...
		DisplayIdSet displayIdSet)
	{
		...
		[DataMember(Order = 2)]
		public ConnectionType Connection { get; } = connection;
		...
		public DisplayIdSet DisplayIdSet { get; } = displayIdSet;
	}
	...
	public static IEnumerable<DisplayItem> EnumerateDisplayConfigs()
	{
		...
		foreach (var displayPath in displayPaths)
		{
			...
			yield return new DisplayItem(
				...
				connection: GetConnectionType(deviceName.outputTechnology),
	...
	private static ConnectionType GetConnectionType(DISPLAYCONFIG_VIDEO_OUTPUT_TECHNOLOGY outputTechnology)
	{
		return outputTechnology switch
		{
			DISPLAYCONFIG_VIDEO_OUTPUT_TECHNOLOGY.DISPLAYCONFIG_OUTPUT_TECHNOLOGY_OTHER => ConnectionType.Other,
